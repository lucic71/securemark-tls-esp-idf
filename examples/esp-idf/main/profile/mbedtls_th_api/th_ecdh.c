...
load_public_peer_key(void *p_context, unsigned char *p_pub, size_t publen)
{
    ...
    ret = mbedtls_ecp_point_read_binary(
        &p_ecdh->private_grp, &Q, uncompressed_point_buffer, 65);
    if (ret != 0)
    {
    ...
    ret = mbedtls_ecp_check_pubkey(&p_ecdh->private_grp, &Q);
    if (ret != 0)
    {
    ...
    ret = mbedtls_ecp_copy(&p_ecdh->private_Qp, &Q);
    if (ret != 0)
    {
...
load_private_key(void *p_context, unsigned char *p_private, size_t prilen)
{
    ...
    p_ecdh = (mbedtls_ecdh_context *)p_context;
    p_grp  = &p_ecdh->private_grp;

    ret = mbedtls_mpi_read_binary(&p_ecdh->private_d, p_private, prilen);
    if (ret != 0)
    {
    ...
    mbedtls_ecp_point_init(&p_ecdh->private_Q);
    ret = mbedtls_ecp_mul(
        p_grp,
        &p_ecdh->private_Q, // R <-- this value will be computed as P * m
        &p_ecdh->private_d, // m
        &p_grp->G,  // P
        NULL,
        0);
...
)
{
    ...
    switch (group)
    {
        case EE_P256R1:
            ...
            ret = mbedtls_ecp_group_load(&p_ecdh->private_grp,
